...
export async function hitBucket({ key, limit, windowSec, failClosed = false }) {
  try {
    await dbConnect();
    const now = new Date();
    ...
    console.error(
      `rate limit okunamadı (${failClosed ? "fail-closed" : "fail-open"})`,
      e.message
    );
    return {
      ok: !failClosed,
      ...
      limit,
      windowSec,
      retryAfter: failClosed ? windowSec : 0,
      failOpen: !failClosed,
      failClosed,
    };
  }
